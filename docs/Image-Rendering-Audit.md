...
| `StoryPromo`                | [`StoryPromo`](../src/app/legacy/containers/StoryPromo/index.jsx)                                                                                                                                                                                      | iChef URL and custom resolution ladder; canonical `Image` placeholder when image data is absent. Uses Psammead StoryPromo layout.    | CMS alt/dimensions/attribution, page- and promo-type-specific sizes, lazy by default, AMP.                                       | Image migrated; legacy layout retained pending CAF decision    |
...
| `EpisodeList`               | [`Episode image`](../src/app/legacy/containers/EpisodeList/Image.jsx)                                                                                                                                                                                  | Canonical `Image` with the existing play/duration overlay.                                                                           | 16:9 placeholder, responsive fixed widths, play/duration overlay, Lite-only play UI.                                             | Image migrated; legacy container retained pending CAF decision |
| `OnDemandImage`             | [`OnDemandImage`](../src/app/legacy/containers/OnDemandImage/index.jsx)                                                                                                                                                                                | Generates Programme `$recipe` square URLs and passes the source set to canonical `Image`.                                            | Square 256px source, 128/256/480 candidates plus 1200 on podcast episodes, Lite hidden.                                          | Image migrated; legacy container retained pending CAF decision |
| `PodcastPromo`              | [`shared preparation`](../src/app/legacy/containers/PodcastPromo/shared.js), [`inline`](../src/app/legacy/containers/PodcastPromo/Inline.jsx), [`secondary`](../src/app/legacy/containers/PodcastPromo/SecondaryColumn.jsx)                            | Programme `$recipe` 128/240/480 WebP set; passes the image to canonical `Image`.                                                     | Square image, lazy loading, 228px desktop / 30vw sizing; Inline is Lite hidden.                                                  | Image migrated; legacy container retained pending CAF decision |
| `ImageWithPlaceholder`      | Removed                                                                                                                                                                                                                                                | Legacy adapter removed; no production consumers remain.                                                                              | Superseded by canonical `Image`, including AMP, placeholder, lazy and noscript behavior.                                         | Removed                                                        |
...
```mermaid
flowchart TD
   OnDemandImage --> CanonicalImage[canonical Image]
   PodcastPromo --> CanonicalImage
   StoryPromo --> CanonicalImage
   EpisodeList --> CanonicalImage
```

The production consumers identified in the original audit have now migrated to
the canonical `Image`. The legacy Psammead image and placeholder packages,
their tests, stories and documentation have been removed. No CAF removal
schedule was available in this repository, so the remaining legacy containers
are retained pending an ownership decision.

## CAF disposition

| Component                 | Current dependency                               | Disposition                                                                                       |
| ------------------------- | ------------------------------------------------ | ------------------------------------------------------------------------------------------------- |
| `ImageWithPlaceholder`    | Removed                                          | No CAF reassessment required; the adapter and its legacy dependencies are deleted.                |
| `OnDemandImage`           | Canonical `Image`                                | Image migration complete; reassess only the surrounding legacy container when CAF scope is known. |
| `PodcastPromo`            | Canonical `Image`                                | Image migration complete; reassess only the surrounding legacy container when CAF scope is known. |
| `EpisodeList` image       | Canonical `Image`                                | Image migration complete; reassess only the surrounding legacy container when CAF scope is known. |
| `StoryPromo` presentation | Psammead StoryPromo layout; canonical `Image`    | Image migration complete; reassess the surrounding legacy presentation when CAF scope is known.   |
...
7. Reassess the remaining legacy container layouts when CAF ownership and
   rollout scope are confirmed; image rendering no longer depends on Psammead.
