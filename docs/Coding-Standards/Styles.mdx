...
.link {
  @include theme.typography-from-scale-and-variant(pica, 'serif-bold');
...
.label {
  @include theme.typography-from-scale-and-variant(brevier, 'sans-regular');
...
Service-specific brand/theme values (e.g. brand colours, font families, font scripts) are set as **CSS custom properties on `:root`** by each service's theme module (see `ThemeProviderSCSSModules/themes/<service>/`). Access typography through `theme.typography-from-scale-and-variant(pica, 'sans-regular')`, which emits the responsive size and variant declarations together. The lower-level size and variant mixins remain available when a component intentionally sets only one of them.
