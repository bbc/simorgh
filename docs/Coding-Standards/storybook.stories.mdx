...
Storybook is a library developed and maitained by [chromatic](https://www.chromatic.com/?utm_source=storybook_website&utm_medium=link&utm_campaign=storybook).
...
Furthermore, we use Storybook to a11y swarm components that cannot be deployed to test/live environements.

The latest update of our Storybook included the addition of the docs addon which allows to preview documentation in Storybook. Thus, other than what we used to
do in the past, storybook has now become our main entry point to Simorgh's documentation.

## Storybook Tutorial

### Introduction - Essentials

Storybook works with path glob patterns that point to stories files. The globs patterns can be found in `.storybook/main.js`:
...
for any of our stories or documentation to correctly display in storybook we must ensure that the folder where they are stored is pointed by any of the above paths glob patterns.
...
To create a story for one of your components, co-locate a file called `index.stories.tsx` where your componnet's entry point (index.tsx/jsx) is located.
...
If your component has different behaviours depending on the props passed to it, you can create more variants with different props passed to it. All these variants will be disaplyed in the component's story.
...
### Creating story for a component that rquires providers
...
To create a didebar windows with Figma or Zepplin UX designs, add the following `array of objects` to the parameters of your `index.stories.jsx` export:
...
To get Figma Urls that points to specific section of a design, select the design you want to display and copy the link showed by the browser:
...
Create and export a left-to-right and right-to-left variat:
...
Most of BBC staff in our team only understand English. With this premisis it should be clear that writing our story in different language might
...
At this stage, Yyu should not worry about this difference as storybook automatically takes care of applying the correct container.
