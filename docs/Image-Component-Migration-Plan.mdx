...
The complete production inventory, legacy dependency graph, CAF disposition,
risks, and validation matrix are maintained in the
[Image Rendering Audit](./Image-Rendering-Audit.md).

The proposed shared-preparation API and BFF transition boundary are documented
in the [Image Preparation API Proposal](./Image-Preparation-API-Proposal.mdx).
...
1. **Audit** — complete. The detailed evidence and agreed decisions are in the
   [Image Rendering Audit](./Image-Rendering-Audit.md).
2. **Define the shared API** — review and agree the
   [Image Preparation API Proposal](./Image-Preparation-API-Proposal.mdx).
   `Image` remains renderer-only; source preparation stays a standalone
   utility that can transition from iChef inputs to BFF renditions.
...
7. **Review Psammead dependencies** — ✅ done. All production image consumers
   now render through the canonical `Image`; the legacy image packages and
   `ImageWithPlaceholder` have been removed. Remaining CAF decisions concern
   legacy container/layout ownership, not image rendering dependencies.
