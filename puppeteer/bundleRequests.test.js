            ...
            it('only loads expected js bundles', () => {
              ...
              requests
                ...
                .forEach(url => {
                  expect(url).toMatch(
                    new RegExp(
                      `(\\/static\\/js\\/(?:comscore\\/)?(modern.)?(main|framework|commons|shared|${serviceRegex}|frosted_promo|CloudWatchRUM|.+Page).+?.js)|(\\/static\\/.+?-lib.+?.js)|${config[service].name}\\/(articles\\/)?sw\\.js`,
                      'g',
                    ),
                  );
                });
