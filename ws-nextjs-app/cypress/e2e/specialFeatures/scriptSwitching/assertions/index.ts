...
const assertScriptSwitchButton = ({
  ...
  const scriptLink = serviceConfigs[service]?.[variant]?.scriptLink;
  const { text: scriptToSwitchToText = '', variant: scriptToSwitchTo = '' } =
    scriptLink ?? {};
...
const assertURLContains = ({ service, variant }: ServiceParametersType) => {
  cy.url().should(url => {
...
const assertLang = ({
  ...
  cy.get('html')
    ...
    .then($lang => {
      // @ts-expect-error - Property 'toLowerCase' does not exist on type 'JQuery<HTMLHtmlElement>'
      expect($lang.toLowerCase()).to.equal(expectedLang);
    });
};

const allVariantAssertions = ({ service, variant }: ServiceParametersType) => {
