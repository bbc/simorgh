...
- The index.js of each page type directory must use the [runTestsForPage function](../support/helpers/runTestsForPage.js), this:
  ...
  - runs tests on every service automatically based on the service:page type combinations found in the [service config files](../support/config/settings).
...
- Inside each of the files in the list above there are three sets of tests:
  ...
  - **testsThatFollowSmokeTestConfig** - this is where most tests will go. These will only run on PRs if the `smoke` value is `true` for that service/pageType combination. [service config files](../support/config/settings)
