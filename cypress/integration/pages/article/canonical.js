...
import { hasArticlePage } from '../../../support/serviceHasPageType';
import { getArticleUrl } from '../../../support/getPageTypeUrl';
...
const runTests = service => {
  describe(`Article - Canonical - ${service}`, () => {
    before(() => {
      cy.visit(getArticleUrl(service));
    });
    ...
    describe(`Meta Tests`, () => {
      ...
      it('should have the correct open graph metadata', () => {
        cy.checkOpenGraphMetadata(
          ...
          'article',
          `https://www.bbc.com${getArticleUrl(service)}`,
        );
      });
      ...
      it('should include the canonical URL & ampHTML', () => {
        cy.checkCanonicalURL(`https://www.bbc.com${getArticleUrl(service)}`);
        cy.checkAmpHTML(
          `${window.location.origin}${getArticleUrl(service)}.amp`,
        );
      });
