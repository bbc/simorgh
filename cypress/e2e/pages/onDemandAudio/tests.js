...
export default ({ service, variant, isAmp }) => {
  describe(
    'Audio Player',
    {
      retries: 3,
  ...
  describe('Recent Episodes component', () => {
    ...
    it('should be displayed if the toggle is on, and shows the expected number of items', () => {
      let toggleName;
      ...
      cy.fixture(`toggles/${service}.json`).then(toggles => {
        const recentEpisodesEnabled = path([toggleName, 'enabled'], toggles);
        ...
        if (recentEpisodesEnabled) {
          const recentEpisodesMaxNumber = path([toggleName, 'value'], toggles);
          ...
          cy.request(getDataUrl(url)).then(({ body }) => {
            ...
            cy.log(`Number of available episodes? ${expectedNumberOfEpisodes}`);
              ...
              if (expectedNumberOfEpisodes > 1) {
                ...
                cy.get('[data-e2e=recent-episodes-list]').within(() => {
                  cy.get('[data-e2e=recent-episodes-list-item]')
                    .its('length')
                    .should(length => {
                      expect(length).to.be.closeTo(expectedNumberOfEpisodes, 1);
                    });
                });
              }
