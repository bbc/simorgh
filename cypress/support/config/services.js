const services = {
  arabic: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: '/arabic/articles/c69dvq19k63o',
        smoke: false,
      },
      errorPage404: {
        path: '/arabic/articles/c123456abcdo',
        smoke: false,
      },
      frontPage: {
        path: 'undefined',
        smoke: false,
      },
      liveRadio: {
        path: 'undefined',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  news: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: '/news/articles/cn7k01xp8kxo',
        smoke: false,
      },
      errorPage404: {
        path: '/news/articles/cxvxrj8tvppo',
        smoke: false,
      },
      frontPage: {
        path: 'undefined',
        smoke: false,
      },
      liveRadio: {
        path: 'undefined',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  pashto: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: '/pashto/articles/cng0e8v85eko',
        smoke: false,
      },
      errorPage404: {
        path: '/pashto/articles/c123456abcdo',
        smoke: false,
      },
      frontPage: {
        path: 'undefined',
        smoke: false,
      },
      liveRadio: {
        path: 'undefined',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  persian: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: '/persian/articles/c4vlle3q337o',
        smoke: false,
      },
      errorPage404: {
        path: '/persian/articles/cxvxrj8tvppo',
        smoke: false,
      },
      frontPage: {
        path: 'undefined',
        smoke: false,
      },
      liveRadio: {
        path: 'undefined',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  urdu: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: '/urdu/articles/cx621klkm1ro',
        smoke: false,
      },
      errorPage404: {
        path: '/urdu/articles/c123456abcdo',
        smoke: false,
      },
      frontPage: {
        path: 'undefined',
        smoke: false,
      },
      liveRadio: {
        path: 'undefined',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  afaanoromoo: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: '/afaanoromoo/articles/c4g19kgl85ko',
        smoke: false,
      },
      errorPage404: {
        path: '/afaanoromoo/articles/c123456abcdo',
        smoke: false,
      },
      frontPage: {
        path: 'undefined',
        smoke: false,
      },
      liveRadio: {
        path: '/afaanoromoo/bbc_afaanoromoo_radio/liveradio',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  afrique: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: '/afrique/articles/cz216x22106o',
        smoke: false,
      },
      errorPage404: {
        path: '/afrique/articles/c123456abcdo',
        smoke: false,
      },
      frontPage: {
        path: 'undefined',
        smoke: false,
      },
      liveRadio: {
        path: 'undefined',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  azeri: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: '/azeri/articles/c5k08pqnzexo',
        smoke: false,
      },
      errorPage404: {
        path: '/azeri/articles/c123456abcdo',
        smoke: false,
      },
      frontPage: {
        path: '/azeri',
        smoke: false,
      },
      liveRadio: {
        path: 'undefined',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  gahuza: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: '/gahuza/articles/cey23zx8wx8o',
        smoke: false,
      },
      errorPage404: {
        path: '/gahuza/articles/c123456abcdo',
        smoke: false,
      },
      frontPage: {
        path: 'undefined',
        smoke: false,
      },
      liveRadio: {
        path: 'undefined',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  hausa: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: '/hausa/articles/c2nr6xqmnewo',
        smoke: false,
      },
      errorPage404: {
        path: '/hausa/articles/c123456abcdo',
        smoke: false,
      },
      frontPage: {
        path: 'undefined',
        smoke: false,
      },
      liveRadio: {
        path: 'undefined',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  igbo: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: '/igbo/articles/cr1lw620ygjo',
        smoke: false,
      },
      errorPage404: {
        path: '/igbo/articles/cxvxrj8tvppo',
        smoke: false,
      },
      frontPage: {
        path: '/igbo',
        smoke: false,
      },
      liveRadio: {
        path: '/igbo/bbc_igbo_radio',
        smoke: false,
      },
      mediaAssetPage: {
        path: '/igbo/media-42986440',
        smoke: false,
      },
    },
  },
  indonesia: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: '/indonesia/articles/c0q2zq8pzvzo',
        smoke: false,
      },
      errorPage404: {
        path: '/indonesia/articles/c123456abcdo',
        smoke: false,
      },
      frontPage: {
        path: 'undefined',
        smoke: false,
      },
      liveRadio: {
        path: '/indonesia/bbc_indonesian_radio/liveradio',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  kyrgyz: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: '/kyrgyz/articles/c3xd4xg3rm9o',
        smoke: false,
      },
      errorPage404: {
        path: '/kyrgyz/articles/c123456abcdo',
        smoke: false,
      },
      frontPage: {
        path: '/kyrgyz',
        smoke: false,
      },
      liveRadio: {
        path: 'undefined',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  mundo: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: '/mundo/articles/c45965g63xno',
        smoke: false,
      },
      errorPage404: {
        path: '/mundo/articles/c123456abcdo',
        smoke: false,
      },
      frontPage: {
        path: 'undefined',
        smoke: false,
      },
      liveRadio: {
        path: 'undefined',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  pidgin: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: '/pidgin/articles/cwl08rd38l6o',
        smoke: false,
      },
      errorPage404: {
        path: '/pidgin/articles/cxvxrj8tvppo',
        smoke: false,
      },
      frontPage: {
        path: '/pidgin',
        smoke: false,
      },
      liveRadio: {
        path: '/pidgin/bbc_pidgin_radio',
        smoke: false,
      },
      mediaAssetPage: {
        path: '/pidgin/23248703',
        smoke: false,
      },
    },
  },
  russian: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: '/russian/articles/ck7pz7re3zgo',
        smoke: false,
      },
      errorPage404: {
        path: '/russian/articles/c123456abcdo',
        smoke: false,
      },
      frontPage: {
        path: 'undefined',
        smoke: false,
      },
      liveRadio: {
        path: 'undefined',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  scotland: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: '/scotland/articles/czwj5l0n210o',
        smoke: false,
      },
      errorPage404: {
        path: '/scotland/articles/cabcdefghijo',
        smoke: false,
      },
      frontPage: {
        path: 'undefined',
        smoke: false,
      },
      liveRadio: {
        path: 'undefined',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  somali: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: '/somali/articles/cgn6emk3jm8o',
        smoke: false,
      },
      errorPage404: {
        path: '/somali/articles/c123456abcdo',
        smoke: false,
      },
      frontPage: {
        path: 'undefined',
        smoke: false,
      },
      liveRadio: {
        path: 'undefined',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  swahili: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: '/swahili/articles/czjqge2jwn2o',
        smoke: false,
      },
      errorPage404: {
        path: '/swahili/articles/c123456abcdo',
        smoke: false,
      },
      frontPage: {
        path: 'undefined',
        smoke: false,
      },
      liveRadio: {
        path: 'undefined',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  turkce: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: '/turkce/articles/cr2d32lwww2o',
        smoke: false,
      },
      errorPage404: {
        path: '/turkce/articles/c123456abcdo',
        smoke: false,
      },
      frontPage: {
        path: 'undefined',
        smoke: false,
      },
      liveRadio: {
        path: 'undefined',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  vietnamese: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: '/vietnamese/articles/c3y59g5zm19o',
        smoke: false,
      },
      errorPage404: {
        path: '/vietnamese/articles/c123456abcdo',
        smoke: false,
      },
      frontPage: {
        path: 'undefined',
        smoke: false,
      },
      liveRadio: {
        path: 'undefined',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  yoruba: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: '/yoruba/articles/clw06m0nj8qo',
        smoke: false,
      },
      errorPage404: {
        path: '/yoruba/articles/cxvxrj8tvppo',
        smoke: false,
      },
      frontPage: {
        path: '/yoruba',
        smoke: false,
      },
      liveRadio: {
        path: '/yoruba/bbc_yoruba_radio',
        smoke: false,
      },
      mediaAssetPage: {
        path: '/yoruba/media-42985961',
        smoke: false,
      },
    },
  },
  japanese: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: 'undefined',
        smoke: false,
      },
      errorPage404: {
        path: 'undefined',
        smoke: false,
      },
      frontPage: {
        path: '/japanese',
        smoke: false,
      },
      liveRadio: {
        path: 'undefined',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  punjabi: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: 'undefined',
        smoke: false,
      },
      errorPage404: {
        path: 'undefined',
        smoke: false,
      },
      frontPage: {
        path: '/punjabi',
        smoke: false,
      },
      liveRadio: {
        path: 'undefined',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  thai: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: 'undefined',
        smoke: false,
      },
      errorPage404: {
        path: 'undefined',
        smoke: false,
      },
      frontPage: {
        path: '/thai',
        smoke: false,
      },
      liveRadio: {
        path: 'undefined',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  portuguese: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: 'undefined',
        smoke: false,
      },
      errorPage404: {
        path: '/portuguese/articles/cxvxrj8tvppo',
        smoke: false,
      },
      frontPage: {
        path: 'undefined',
        smoke: false,
      },
      liveRadio: {
        path: 'undefined',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  amharic: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: 'undefined',
        smoke: false,
      },
      errorPage404: {
        path: 'undefined',
        smoke: false,
      },
      frontPage: {
        path: 'undefined',
        smoke: false,
      },
      liveRadio: {
        path: '/amharic/bbc_amharic_radio/liveradio',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  korean: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: 'undefined',
        smoke: false,
      },
      errorPage404: {
        path: 'undefined',
        smoke: false,
      },
      frontPage: {
        path: 'undefined',
        smoke: false,
      },
      liveRadio: {
        path: '/korean/bbc_korean_radio/liveradio',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
  tigrinya: {
    font: 'undefined',
    isWorldService: true,
    variant: 'default',
    pageTypes: {
      articles: {
        path: 'undefined',
        smoke: false,
      },
      errorPage404: {
        path: 'undefined',
        smoke: false,
      },
      frontPage: {
        path: 'undefined',
        smoke: false,
      },
      liveRadio: {
        path: '/tigrinya/bbc_tigrinya_radio/liveradio',
        smoke: false,
      },
      mediaAssetPage: {
        path: 'undefined',
        smoke: false,
      },
    },
  },
};

// Allow runs to be limited to a single service via the CYPRESS_ONLY_SERVICE env var
const runOnlyService = Cypress.env('ONLY_SERVICE');
if (runOnlyService && Object.keys(services).includes(runOnlyService)) {
  module.exports = { [runOnlyService]: services[runOnlyService] };
} else if (Cypress.env('APP_ENV') === 'stage') {
  module.exports = services;
} else {
  module.exports = services;
}
