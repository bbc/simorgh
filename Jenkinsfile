...
pipeline {
  ...
  stages {
    ...
    stage ('Run application tests') {
      ...
      steps {
        sh 'make install'
        sh 'LIGHTHOUSE_CHROME_PATH="$(pwd)/chrome-linux/chrome" curl https://raw.githubusercontent.com/GoogleChrome/chrome-launcher/v0.8.0/scripts/download-chrome.sh | bash'
