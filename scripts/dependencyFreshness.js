    ...
    await fetchResponse(downloadsBaseUrl + dep).then((response) => {
        ...
        const responseJson = response.json().then((json) => {
            ...
            if (depenencyTable.length === target) {
                let csvContents = 'dependency,Days since last activity,Freshness of our version in days,Downloads Last Week';
                depenencyTable.forEach(({name, freshness, ourfreshness, weeklyDownloads}) => {
...
Object.keys(allDependencies).forEach((dep, i) => {
    ...
    const cmd = exec(`npm view ${dep} time --json`, (err, stdout, stderr) => {
        ...
        const ourVersion = dealWithCaretsAndTildes(allDependencies[dep], stdoutJson);
