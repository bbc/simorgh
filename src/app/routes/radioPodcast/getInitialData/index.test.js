...
describe('Get initial data for on demand radio', () => {
  ...
  it('should return essential data for a page to render', async () => {
    ...
    expect(podcastPageData.headline).toEqual('BBC Xtra');
    expect(podcastPageData.releaseDateTimeStamp).toEqual(1603929600000);
    expect(podcastPageData.summary).toEqual(
      'التصويت عبر البريد في الانتخابات الرئاسية الأميركية',
    );
    expect(podcastPageData.language).toEqual('ar');
    expect(podcastPageData.metadata.type).toEqual('Podcast');
    expect(podcastPageData.imageUrl).toEqual(
      'ichef.bbci.co.uk/images/ic/$recipe/p02rt7vj.jpg',
    );
    expect(podcastPageData.promoBrandTitle).toEqual('BBC Xtra');
    expect(podcastPageData.durationISO8601).toEqual('PT6M50S');
    expect(podcastPageData.thumbnailImageUrl).toEqual(
