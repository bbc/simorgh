...
import createUASStatusHook, { UASStatusField } from './createUASStatusHook';
...
describe('createUASStatusHook', () => {
  ...
  describe('followed topics context', () => {
    const useTopicStatus = createUASStatusHook({
      config: {
    ...
    it('returns isFollowed and metadata from query data', () => {
      ...
      const { result } = renderHook(() => useTopicStatus('topic-1'));
    ...
    it('calls UAS GET request with built globalId and refresh flag', async () => {
      ...
      renderHook(() => useTopicStatus('topic-1'));
      await mockQueryFn();
  ...
  describe('saved article context', () => {
    const useArticleStatus = createUASStatusHook({
      ...
      queryKeyFn: (hashedUserId, articleId) => ['uas', hashedUserId, articleId],
    ...
    it('returns isSaved from query data when using saved-article config', () => {
      ...
      const { result } = renderHook(() => useArticleStatus('article-123'));
    ...
    it('calls UAS favourites endpoint with article globalId', async () => {
      ...
      renderHook(() => useArticleStatus('article-123'));
      await mockQueryFn();
    ...
    it('uses enabledFn when provided', () => {
      const customHook = createUASStatusHook({
        config: {
          activityType: 'favourites',
          resourceDomain: 'world-service-news',
          resourceType: 'article',
      ...
      renderHook(() => customHook('article-1'));
