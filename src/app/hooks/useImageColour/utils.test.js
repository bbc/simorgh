...
const buildPalette = options => ({
  Vibrant: { hex: options[0], rgb: hexToRgb(options[0]) },
  Muted: { hex: options[1], rgb: hexToRgb(options[1]) },
  SomethingElse: { hex: options[2], rgb: hexToRgb(options[2]) },
});
...
describe('selectColour', () => {
  it.each`
    ...
    ${3}     | ${[]}                       | ${1}            | ${'#000'}      | ${'fallback'}
    ${4}     | ${['#f00', '#ff0', '#fff']} | ${0}            | ${'#000'}      | ${'#f00'}
    ${5}     | ${['#fff', '#ff0', '#f00']} | ${0}            | ${'#000'}      | ${'#fff'}
    ${6}     | ${['#f00', '#ff0', '#fff']} | ${5}            | ${'#000'}      | ${'#f00'}
    ${7}     | ${['#f00', '#ff0', '#fff']} | ${15}           | ${'#000'}      | ${'#ff0'}
    ${8}     | ${['#f00', '#ff0', '#fff']} | ${20}           | ${'#000'}      | ${'#fff'}
  ...
  `(
    ...
    ({ options, minimumContrast, contrastColour, expectedResult }) => {
      const palette = options ? buildPalette(options) : {};
