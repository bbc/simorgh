...
| alwaysInView      | boolean | no       | Use this to track components that are always in view and such do not be observed by the IntersectionObserver. This flag is use as an alternative to `eventSent` state variable which is used to track components that require the use to scroll into view. |
...
const TopStories = () => {
  ...
  const TopStory = ({ link, title, viewTracker }) => (
    <li {...viewTracker}>
      <a href={link}>{title}</a>
    </li>
  );
  ...
  return (
    <ol>
      {topStories.map(({ title, link }) => (
        <TopStory {...viewTracker} title={title} link={link} />
      ))}
    </ol>
  );
};
