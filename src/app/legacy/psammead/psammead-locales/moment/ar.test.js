...
test.only('parse', () => {
  ...
  console.log(dayjs(tests[0]));

  function equalTest(input, mmm, i) {
    // console.log(input, mmm);
    // console.log(dayjs(input, mmm));
    // console.log(dayjs(input));
    assert.equal(
      dayjs(input, mmm).month(),
      i,
      `${input} should be month ${i + 1} instead is month ${dayjs(
        input,
        mmm
      ).month()}`
    );
...
test('format', () => {
  const a = [
    ...
    ['ll', '14 فبراير/ شباط 2010'],
    ['lll', '14 فبراير/ شباط 2010 15:25'],
    ['llll', 'أحد 14 فبراير/ شباط 2010 15:25'],
  ];
  const b = dayjs(new Date(2010, 1, 14, 15, 25, 50, 125));
  let i;
...
test('format ordinal', () => {
  assert.equal(dayjs([2011, 0, 1]).format('DDDo'), '1', '1');
  assert.equal(dayjs([2011, 0, 2]).format('DDDo'), '2', '2');
  assert.equal(dayjs([2011, 0, 3]).format('DDDo'), '3', '3');
  assert.equal(dayjs([2011, 0, 4]).format('DDDo'), '4', '4');
  assert.equal(dayjs([2011, 0, 5]).format('DDDo'), '5', '5');
  assert.equal(dayjs([2011, 0, 6]).format('DDDo'), '6', '6');
  assert.equal(dayjs([2011, 0, 7]).format('DDDo'), '7', '7');
  assert.equal(dayjs([2011, 0, 8]).format('DDDo'), '8', '8');
  assert.equal(dayjs([2011, 0, 9]).format('DDDo'), '9', '9');
  assert.equal(dayjs([2011, 0, 10]).format('DDDo'), '10', '10');

  assert.equal(dayjs([2011, 0, 11]).format('DDDo'), '11', '11');
  assert.equal(dayjs([2011, 0, 12]).format('DDDo'), '12', '12');
  assert.equal(dayjs([2011, 0, 13]).format('DDDo'), '13', '13');
  assert.equal(dayjs([2011, 0, 14]).format('DDDo'), '14', '14');
  assert.equal(dayjs([2011, 0, 15]).format('DDDo'), '15', '15');
  assert.equal(dayjs([2011, 0, 16]).format('DDDo'), '16', '16');
  assert.equal(dayjs([2011, 0, 17]).format('DDDo'), '17', '17');
  assert.equal(dayjs([2011, 0, 18]).format('DDDo'), '18', '18');
  assert.equal(dayjs([2011, 0, 19]).format('DDDo'), '19', '19');
  assert.equal(dayjs([2011, 0, 20]).format('DDDo'), '20', '20');

  assert.equal(dayjs([2011, 0, 21]).format('DDDo'), '21', '21');
  assert.equal(dayjs([2011, 0, 22]).format('DDDo'), '22', '22');
  assert.equal(dayjs([2011, 0, 23]).format('DDDo'), '23', '23');
  assert.equal(dayjs([2011, 0, 24]).format('DDDo'), '24', '24');
  assert.equal(dayjs([2011, 0, 25]).format('DDDo'), '25', '25');
  assert.equal(dayjs([2011, 0, 26]).format('DDDo'), '26', '26');
  assert.equal(dayjs([2011, 0, 27]).format('DDDo'), '27', '27');
  assert.equal(dayjs([2011, 0, 28]).format('DDDo'), '28', '28');
  assert.equal(dayjs([2011, 0, 29]).format('DDDo'), '29', '29');
  assert.equal(dayjs([2011, 0, 30]).format('DDDo'), '30', '30');

  assert.equal(dayjs([2011, 0, 31]).format('DDDo'), '31', '31');
});

test('format month', () => {
  const expected = dualMonthNames;
  let i;
  for (i = 0; i < expected.length; i += 1) {
    assert.equal(dayjs([2011, i, 1]).format('MMMM'), expected[i], expected[i]);
    assert.equal(dayjs([2011, i, 1]).format('MMM'), expected[i], expected[i]);
  }
});
...
test('from', () => {
  ...
  assert.equal(
    start.from(dayjs([2007, 1, 28]).add({ d: 43 }), true),
    'شهر واحد',
    '43 days = a month'
  );
  assert.equal(
...
test('suffix', () => {
  assert.equal(dayjs(30000).from(0), 'بعد 30 ثانية', 'prefix');
  assert.equal(dayjs(0).from(30000), '30 ثانية', 'suffix');
});
...
test('fromNow', () => {
  ...
  assert.equal(dayjs().add({ d: 5 }).fromNow(), 'بعد 5 أيام', 'in 5 days');
});
...
test('weeks year starting wednesday custom', () => {
  assert.equal(
    dayjs('2003 1 6', 'gggg w d').format('YYYY-MM-DD'),
    '2002-12-28',
    'Week 1 of 2003 should be Dec 28 2002'
  );
  assert.equal(
    dayjs('2003 1 0', 'gggg w e').format('YYYY-MM-DD'),
    '2002-12-28',
    'Week 1 of 2003 should be Dec 28 2002'
  );
  assert.equal(
    dayjs('2003 1 6', 'gggg w d').format('gggg w d'),
    '2003 1 6',
    'Saturday of week 1 of 2003 parsed should be formatted as 2003 1 6'
  );
  assert.equal(
    dayjs('2003 1 0', 'gggg w e').format('gggg w e'),
    '2003 1 0',
...
test('weeks year starting sunday formatted', () => {
  assert.equal(
    dayjs([2011, 11, 31]).format('w ww wo'),
    '1 01 1',
    'Dec 31 2011 should be week 1'
  );
  assert.equal(
    dayjs([2012, 0, 6]).format('w ww wo'),
    '1 01 1',
    'Jan  6 2012 should be week 1'
  );
  assert.equal(
    dayjs([2012, 0, 7]).format('w ww wo'),
    '2 02 2',
    'Jan  7 2012 should be week 2'
  );
  assert.equal(
    dayjs([2012, 0, 13]).format('w ww wo'),
    '2 02 2',
    'Jan 13 2012 should be week 2'
  );
  assert.equal(
    dayjs([2012, 0, 14]).format('w ww wo'),
    '3 03 3',
...
test('no leading zeros in long date formats', () => {
  ...
  for (i = 1; i <= 9; i += 1) {
    for (j = 1; j <= 9; j += 1) {
      longDateStr = dayjs([2014, i, j]).format('L');
      shortDateStr = dayjs([2014, i, j]).format('l');
      assert.equal(
        longDateStr,
        shortDateStr,
...
test('ar strict mode parsing works', () => {
  const m = dayjs().locale('ar');
  const formattedDate = m.format('l');
  assert.equal(
    dayjs.utc(formattedDate, 'l', 'ar', false).isValid(),
    true,
    'Non-strict parsing works'
  );
  assert.equal(
    dayjs.utc(formattedDate, 'l', 'ar', true).isValid(),
    true,
