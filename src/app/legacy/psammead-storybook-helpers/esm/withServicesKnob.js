...
export default (function () {
  ...
  return function (storyFn) {
    var selectedService = select('Select a service', services.filter(includesService(services)), defaultService);
    var variant = getVariant(selectedService)(TEXT_VARIANTS);
    var service = variant ? getService(selectedService)(TEXT_VARIANTS) : selectedService;
