"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.default = void 0;

var _base = _interopRequireDefault(require("@emotion/styled/base"));

var _react = _interopRequireDefault(require("react"));

var _propTypes = require("prop-types");

var _colours = require("@bbc/psammead-styles/colours");

var _spacings = require("@bbc/gel-foundations/spacings");

var _breakpoints = require("@bbc/gel-foundations/breakpoints");

var _detection = require("@bbc/psammead-styles/detection");

var _Link = _interopRequireDefault(require("../Link"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

// Gets the number of grid rows, taking into account the
// trustProjectLink in the grid being separate, on its own row.
var getRowCount = function getRowCount(links, columns, trustProjectLink) {
  return trustProjectLink ? Math.ceil(links.length / columns) + 1 : Math.ceil(links.length / columns);
};

var StyledList = (0, _base.default)("ul", process.env.NODE_ENV === "production" ? {
  target: "e1jbzoem1"
} : {
  target: "e1jbzoem1",
  label: "StyledList"
})("border-bottom:0.0625rem solid ", _colours.C_SHADOW, ";list-style-type:none;margin:0;padding:", function (_ref) {
  var trustProjectLink = _ref.trustProjectLink;
  return trustProjectLink ? "0 0 ".concat(_spacings.GEL_SPACING) : "".concat(_spacings.GEL_SPACING, " 0");
}, ";column-count:4;@supports (", _detection.grid, "){display:grid;grid-auto-flow:column;}@media (max-width: ", _breakpoints.GEL_GROUP_0_SCREEN_WIDTH_MAX, "){grid-auto-flow:row;column-count:1;}@media (min-width: ", _breakpoints.GEL_GROUP_1_SCREEN_WIDTH_MIN, ") and (max-width: ", _breakpoints.GEL_GROUP_2_SCREEN_WIDTH_MAX, "){grid-column-gap:", _spacings.GEL_SPACING, ";grid-template-columns:repeat(2, 1fr);grid-template-rows:repeat(\n      ", function (_ref2) {
  var links = _ref2.links,
      trustProjectLink = _ref2.trustProjectLink;
  return getRowCount(links, 2, trustProjectLink);
}, ",\n      auto\n    );column-count:2;}@media (min-width: ", _breakpoints.GEL_GROUP_3_SCREEN_WIDTH_MIN, ") and (max-width: ", _breakpoints.GEL_GROUP_3_SCREEN_WIDTH_MAX, "){grid-column-gap:", _spacings.GEL_SPACING_DBL, ";grid-template-columns:repeat(3, 1fr);grid-template-rows:repeat(\n      ", function (_ref3) {
  var links = _ref3.links,
      trustProjectLink = _ref3.trustProjectLink;
  return getRowCount(links, 3, trustProjectLink);
}, ",\n      auto\n    );column-count:3;}@media (min-width: ", _breakpoints.GEL_GROUP_4_SCREEN_WIDTH_MIN, ") and (max-width: ", _breakpoints.GEL_GROUP_4_SCREEN_WIDTH_MAX, "){grid-column-gap:", _spacings.GEL_SPACING_DBL, ";grid-template-columns:repeat(4, 1fr);grid-template-rows:repeat(\n      ", function (_ref4) {
  var links = _ref4.links,
      trustProjectLink = _ref4.trustProjectLink;
  return getRowCount(links, 4, trustProjectLink);
}, ",\n      auto\n    );column-count:4;}@media (min-width: ", _breakpoints.GEL_GROUP_5_SCREEN_WIDTH_MIN, "){grid-column-gap:", _spacings.GEL_SPACING_DBL, ";grid-template-columns:repeat(5, 1fr);grid-template-rows:repeat(\n      ", function (_ref5) {
  var links = _ref5.links,
      trustProjectLink = _ref5.trustProjectLink;
  return getRowCount(links, 5, trustProjectLink);
}, ",\n      auto\n    );column-count:5;}", function (_ref6) {
  var trustProjectLink = _ref6.trustProjectLink;
  return trustProjectLink && "> li:first-of-type {\n    border-bottom: 0.0625rem solid ".concat(_colours.C_SHADOW, ";\n    padding: ").concat(_spacings.GEL_SPACING, " 0;\n    margin-bottom: ").concat(_spacings.GEL_SPACING, ";\n    grid-column: 1/-1;\n    width: 100%;\n    column-span: all;\n  }");
}, ";" + (process.env.NODE_ENV === "production" ? "" : "/*# sourceMappingURL=data:application/json;charset=utf-8;base64, */"));
var StyledListItem = (0, _base.default)("li", process.env.NODE_ENV === "production" ? {
  target: "e1jbzoem0"
} : {
  target: "e1jbzoem0",
  label: "StyledListItem"
})("min-width:50%;column-gap:", _spacings.GEL_SPACING_DBL, ";break-inside:avoid-column;" + (process.env.NODE_ENV === "production" ? "" : "/*# sourceMappingURL=data:application/json;charset=utf-8;base64, */"));

var listItem = function listItem(key, text, href, lang) {
  return /*#__PURE__*/_react.default.createElement(StyledListItem, {
    key: key,
    role: "listitem"
  }, /*#__PURE__*/_react.default.createElement(_Link.default, {
    text: text,
    href: href,
    lang: lang
  }));
};

var List = function List(_ref7) {
  var links = _ref7.links,
      trustProjectLink = _ref7.trustProjectLink;
  return /*#__PURE__*/_react.default.createElement(StyledList, {
    role: "list",
    trustProjectLink: trustProjectLink,
    links: links
  }, trustProjectLink && listItem(trustProjectLink.text, trustProjectLink.text, trustProjectLink.href), links.map(function (link) {
    return listItem(link.text, link.text, link.href, link.lang);
  }));
};

var linkPropTypes = (0, _propTypes.shape)({
  href: _propTypes.string.isRequired,
  text: _propTypes.string.isRequired,
  lang: _propTypes.string
});
List.propTypes = {
  links: (0, _propTypes.arrayOf)(linkPropTypes.isRequired).isRequired,
  trustProjectLink: linkPropTypes
};
List.defaultProps = {
  trustProjectLink: null
};
var _default = List;
exports.default = _default;