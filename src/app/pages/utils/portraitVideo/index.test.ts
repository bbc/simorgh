...
describe('Portrait Video', () => {
  ...
  describe('isPortraitVideoUnderHeadline', () => {
    it(`Should return true if media block is a portrait video and is under the headline`, () => {
      const articlePageBlocks = [
        { type: 'headline', model: {} },
        { type: 'video', model: { blocks: mediaBlockWithPV } },
      ];
      expect(
        isPortraitVideoUnderHeadline(articlePageBlocks, mediaBlockWithPV),
      ).toBe(true);
    });
  });
});
