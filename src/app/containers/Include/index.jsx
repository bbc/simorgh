...
const Include = () => {
  ...
  // We know rendering <scripts> work with in SSR, so
  // simulate a client-side fetch using `useEffect` hook.
  useEffect(() => {
    ...
    const scriptTags = new RegExp(/<script\b[^>]*>([\s\S]*?)<\/script>/gm);
    setScripts(Array.from(rawHtml.matchAll(scriptTags)));
    setHtml(rawHtml.replace(scriptTags, ''));
  }, []);

  // When the `scripts` value changes.
  useEffect(() => {
    scripts.forEach(script => {
      // Use the group match, which doesn't include <script> tags.
      const code = script[1];
      appendScript(code);
    });
  }, [scripts]);
