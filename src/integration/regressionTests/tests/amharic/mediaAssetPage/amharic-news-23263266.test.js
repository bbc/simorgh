...
it('I can see a page title - AMP', () => {
  ...
  expect(pageTitle).toMatchInlineSnapshot(
    `"Amharic test MAP A vertical video pretending to be a cat short synopsis - BBC News አማርኛ"`,
  );
});
...
it('I can see a headline - AMP', () => {
  ...
  expect(headline).toMatchInlineSnapshot(
    `"Amharic test MAP A vertical video pretending to be a cat short synopsis"`,
  );
});
