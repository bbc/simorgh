...
it('I can see an audio player embed - Canonical', () => {
  ...
  expect(audioPlayerIframe.getAttribute('src')).toMatchInlineSnapshot(
    `"https://polling.test.bbc.co.uk/ws/av-embeds/media/bbc_bangla_radio/liveradio/bn"`,
  );
});
...
it('I can see an audio player embed - AMP', () => {
  ...
  expect(audioPlayerIframe.getAttribute('src')).toMatchInlineSnapshot(
    `"https://polling.test.bbc.co.uk/ws/av-embeds/media/bbc_bangla_radio/liveradio/bn/amp"`,
  );
});
