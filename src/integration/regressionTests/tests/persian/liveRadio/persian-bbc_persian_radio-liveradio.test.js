...
it('I can see an audio player embed - Canonical', () => {
  ...
  expect(audioPlayerIframe.getAttribute('src')).toMatchInlineSnapshot(
    `"https://polling.test.bbc.co.uk/ws/av-embeds/media/bbc_persian_radio/liveradio/fa"`,
  );
});
...
it('I can see an audio player embed - AMP', () => {
  ...
  expect(audioPlayerIframe.getAttribute('src')).toMatchInlineSnapshot(
    `"https://polling.test.bbc.co.uk/ws/av-embeds/media/bbc_persian_radio/liveradio/fa/amp"`,
  );
});
