...
it('I can see a page title - AMP', () => {
  ...
  expect(pageTitle).toMatchInlineSnapshot(
    `"Bu test məqaləsinin SEO başlığı - BBC News Azərbaycanca"`,
  );
});
...
it('I can see a skip to content link that links to the main content of the page - AMP', () => {
  ...
  expect(skipToContentEl.textContent).toMatchInlineSnapshot(`"Mətnə keçid"`);
});
...
it('I can see a headline - AMP', () => {
  ...
  expect(headline).toMatchInlineSnapshot(`"Bu test məqaləsinin başlığı budur"`);
});
...
it('I can see the footer copyright - AMP', () => {
  ...
  expect(footerCopyright).toMatchInlineSnapshot(
    `"© 2020 BBC. BBC kənar saytların məzmununa məsul deyil. Bizim kənar keçidlərə dair yanaşmamız barədə oxuyun."`,
  );
});
