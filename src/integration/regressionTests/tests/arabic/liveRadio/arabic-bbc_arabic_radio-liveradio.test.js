...
it(`I can see the brand logo in the header - AMP`, () => {
  ...
  expect(logo.parentNode.textContent).toMatchInlineSnapshot(`"BBC News, عربي"`);
});
...
it('I can see an audio player embed - Canonical', () => {
  ...
  expect(audioPlayerIframe.getAttribute('src')).toMatchInlineSnapshot(
    `"https://polling.test.bbc.co.uk/ws/av-embeds/media/bbc_arabic_radio/liveradio/ar"`,
  );
});
...
it('I can see an audio player embed - AMP', () => {
  ...
  expect(audioPlayerIframe.getAttribute('src')).toMatchInlineSnapshot(
    `"https://polling.test.bbc.co.uk/ws/av-embeds/media/bbc_arabic_radio/liveradio/ar/amp"`,
  );
});
