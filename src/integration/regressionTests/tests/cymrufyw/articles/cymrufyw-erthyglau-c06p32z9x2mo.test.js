...
it('I can see a page title - AMP', () => {
  ...
  expect(pageTitle).toMatchInlineSnapshot(
    `"Dyma bennawd SEO yr erthygl brawf yma - BBC Cymru Fyw"`,
  );
});
...
it('I can see a headline - AMP', () => {
  ...
  expect(headline).toMatchInlineSnapshot(`"Dyma bennawd yr erthygl brawf yma"`);
});
...
it('I can see the footer copyright - AMP', () => {
  ...
  expect(footerCopyright).toMatchInlineSnapshot(
    `"© 2020 BBC. Nid yw'r BBC yn gyfrifol am gynnwys safleoedd allanol. Sut ry'n ni'n defnyddio dolenni allanol."`,
  );
});
