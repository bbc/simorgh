...
it('I can see a page title - AMP', () => {
  ...
  expect(pageTitle).toMatchInlineSnapshot(`"主頁 - BBC News 中文"`);
});
...
it(`I can see the brand logo in the header - AMP`, () => {
  ...
  expect(logo.parentNode.textContent).toMatchInlineSnapshot(`"BBC News, 中文"`);
});
...
it('I can see a skip to content link that links to the main content of the page - AMP', () => {
  ...
  expect(skipToContentEl.textContent).toMatchInlineSnapshot(`"跳過此內容"`);
});
...
it('I can see a headline - AMP', () => {
  ...
  expect(headline).toMatchInlineSnapshot(`"BBC News, 中文 - 主頁"`);
});
...
it('I can see the footer copyright - AMP', () => {
  ...
  expect(footerCopyright).toMatchInlineSnapshot(
    `"© 2020 BBC. BBC對外部網站內容不負責任。 閱讀了解我們對待外部鏈接的做法。"`,
  );
});
