...
it('I can see a page title - AMP', () => {
  ...
  expect(pageTitle).toMatchInlineSnapshot(
    `"Este artigo de teste foi criado para que possamos executar testes de ponta a ponta automatizados - BBC News Brasil"`,
  );
});
...
it('I can see a headline - AMP', () => {
  ...
  expect(headline).toMatchInlineSnapshot(
    `"Este é o título deste artigo de teste"`,
  );
});
...
it('I can see the footer copyright - AMP', () => {
  ...
  expect(footerCopyright).toMatchInlineSnapshot(
    `"© 2020 BBC. A BBC não se responsabiliza pelo conteúdo de sites externos. Leia sobre nossa política em relação a links externos."`,
  );
});
