/**
 * @pathname /turkce/multimedya/2016/05/160524_tc2_testmap1
 */
...
it('I can see a page title - AMP', () => {
  ...
  expect(pageTitle).toMatchInlineSnapshot(
    `"test map IŞİD'e direnen Rakkalı gençler 2: Mücadelenin bedeli - BBC News Türkçe"`,
  );
});
...
it('I can see a skip to content link that links to the main content of the page - AMP', () => {
  ...
  expect(skipToContentEl.textContent).toMatchInlineSnapshot(`"İçeriğe götür"`);
});
...
it('I can see a headline - AMP', () => {
  ...
  expect(headline).toMatchInlineSnapshot(
    `"test map IŞİD'e direnen Rakkalı gençler 2: Mücadelenin bedeli"`,
  );
});
...
it('I can see the footer copyright - AMP', () => {
  ...
  expect(footerCopyright).toMatchInlineSnapshot(
    `"© 2020 BBC. BBC, link verilen internet sitelerinin içeriğinden sorumlu değildir. Link verilen internet sitelerine yaklaşımımız."`,
  );
});
