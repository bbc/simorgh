...
it('I can see a page title - AMP', () => {
  ...
  expect(pageTitle).toMatchInlineSnapshot(
    `"BBC Uzbek Radio Dasturi - BBC News O'zbek"`,
  );
});
...
it('I can see a headline - AMP', () => {
  ...
  expect(headline).toMatchInlineSnapshot(`"BBC Uzbek Radio Dasturi"`);
});
...
it('I can see the summary - AMP', () => {
  ...
  expect(summaryEl.textContent).toMatchInlineSnapshot(
    `"O’zbekiston, mintaqa va dunyo yangiliklari O’zbek tilida"`,
  );
});
...
it('I can see an audio player embed - Canonical', () => {
  ...
  expect(audioPlayerIframe.getAttribute('src')).toMatchInlineSnapshot(
    `"https://polling.test.bbc.co.uk/ws/av-embeds/media/bbc_uzbek_radio/liveradio/uz-Cyrl"`,
  );
});
...
it('I can see an audio player embed - AMP', () => {
  ...
  expect(audioPlayerIframe.getAttribute('src')).toMatchInlineSnapshot(
    `"https://polling.test.bbc.co.uk/ws/av-embeds/media/bbc_uzbek_radio/liveradio/uz-Cyrl/amp"`,
  );
});
