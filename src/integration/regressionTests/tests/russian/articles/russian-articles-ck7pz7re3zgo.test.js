...
it('I can see a page title - AMP', () => {
  ...
  expect(pageTitle).toMatchInlineSnapshot(
    `"Пугало полицейского с камерой следит за водителями под Ливерпулем - BBC News Русская служба"`,
  );
});
...
it('I can see a headline - AMP', () => {
  ...
  expect(headline).toMatchInlineSnapshot(
    `"Это заголовок этой тестовой статьи"`,
  );
});
...
it('I can see the footer copyright - AMP', () => {
  ...
  expect(footerCopyright).toMatchInlineSnapshot(
    `"© 2020 BBC. Би-би-си на несет ответственности за содержание других сайтов. Познакомьтесь с нашими правилами внешних ссылок."`,
  );
});
