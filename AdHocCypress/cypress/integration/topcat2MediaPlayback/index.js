...
describe('Legacy MAP Media Playback', () => {
  ...
  if (paths.length > 0) {
    paths.forEach(path => {
      it(`SMP playlist has expected hrefs, bitrates and undefined url - ${path}`, () => {
          ...
          cy.get(
            ...
            cy.wrap($iframe.prop('contentWindow'), {
              ...
              .then(playlist => {
                ...
                const bitrates = originalConnections.map(item => item.bitrate);
      ...
      it(`Player has expected image - ${path}`, () => {
        cy.request(`${path}.json`).then(({ body: jsonData }) => {
          cy.visit(path);
          cy.get(
            'div[class^="StyledVideoContainer"] iframe[class^="StyledIframe"]',
            ...
            cy.wrap($iframe.prop('contentWindow'), {
              ...
              .should(
                'eq',
                jsonData.content.blocks[0].image.href.replace('http', 'https'),
              );
          });
        });
      });
